\documentclass[11pt]{article}
\usepackage[margin=1in]{geometry}
\usepackage{parskip}
\usepackage{graphicx}
\usepackage{url}

\title{EECS 473 Homework 0}
\author{Kevin Jin}
\date{Sep 4, 2026}

\begin{document}
\maketitle

\section*{Proposal 1: Through-Wall Motion Radar}

\subsection*{High-Level Description}

A portable radar that shows you people moving on the other side of a wall. The finished device is a box you hold in one hand (may be unrealistic, backup is two hands). Ideally we'd get the box to be around the size of a thick phone, but that may be unrealistic based on the antenna arrangement we will need to adopt. A patch antenna array points to the "top" of the box, a screen on the front showing a forward sector view. You point it at a wall, press a button, and moving people behind that wall appear as blips that track as they walk around. 

Commercial through-wall radars exist. The Range-R unit used by US police and the Xaver line are two examples, but they cost between six and fifty thousand dollars. The pitch is that a student team can build a functional one for a few hundred dollars in parts (that's with our designs and RnD cost excluded). The users who care are first responders searching the ruins of a collapsed building or tactical teams clearing a structure, both teams would want to know whether a room is occupied before they walk into it. 

The device works as an FMCW (frequency-modulated continuous-wave) radar that the team build themselves, operating somewhere in the 3 to 7 GHz range. The transmitter sends a chirp, the receiver mixes the return against the outgoing signal, and the beat frequency of that mix is proportional to target range. Multiple receive antennas give us angle of arrival by comparing phase across the array. Walls and furniture are static, so we subtract a running background estimate, a technique called moving-target indication, and what remains is the moving people. We render those returns as blips on the display.

% if using, put in another section
% We chose this frequency band deliberately. The 60 GHz mmWave radar modules that are cheap and available off the shelf do not penetrate walls, because attenuation at that frequency is severe. Lower frequencies pass through drywall, plywood, and brick with a manageable loss, and 3 to 7 GHz is the range where enough bandwidth is still available for useful range resolution. MIT's WiTrack project demonstrated through-wall localization of a single moving person in roughly this band with about 1.8 GHz of bandwidth, so the underlying physics is already proven. We are re-implementing a known result on a budget, not inventing a new one.

\subsection*{Implementation}

\subsubsection*{Rough Timeline}

\begin{center}
\small
\begin{tabular}{p{2.3cm} p{12.5cm}}
\hline
9/10--10/1 & Planning and part selection. Choose the PLL/VCO, mixer, LNA, ADC, and processor. Simulate the antenna array. Settle on chirp bandwidth and chirp rate, and finalize this proposal. \\
\hline
10/1--10/22 & Breadboard the RF chain on evaluation modules. One transmit channel, one receive channel. Obtain a clean range measurement in free space and stream raw IQ samples to a laptop. \\
\hline
10/15--11/5 & PCB design and first order. RF front end, processor, and power on a controlled-impedance board, with footprints for the full receive array. Order by roughly 10/26. \\
\hline
10/22--11/19 & DSP pipeline on the laptop. FMCW range processing, moving-target indication, angle of arrival, and the forward-sector visualization. Validate it through a real interior wall. \\
\hline
11/12--12/3 & Board bring-up and integration. Move the DSP onto the processor, or stream to a companion module if necessary. Integrate the screen and enclosure. Order a second board revision if needed. \\
\hline
12/3--12/10 & Final testing, calibration, poster, and Expo. Characterize range and accuracy, and add stretch goals if time allows. \\
\hline
\end{tabular}
\end{center}

\subsubsection*{Rough Budget}

\begin{center}
\small
\begin{tabular}{p{6.5cm} p{5.5cm} c}
\hline
Category & Examples & Est. \$ \\
\hline
RF semiconductors & VCO, PLL, LNAs, mixers, gain blocks & 250 \\
ADC and digital capture & dual 12-bit 40--65 MSPS parts & 100 \\
Processor / FPGA board & Zynq-7020 class & 150 \\
RF connectors, cable, passives & SMA launches, RG405, filters, baluns & 150 \\
PCB fabrication & FR-4 iteration plus final Rogers hybrid & 300 \\
Display, battery, enclosure & 5 in LCD, LiPo, 3D-printed housing & 90 \\
Shipping & several part and board orders & 75 \\
\hline
\textbf{Total} & & \textbf{1115} \\
\hline
\end{tabular}
\end{center}

That leaves roughly \$135 of headroom under the \$1250 ceiling. Substituting a department-owned FPGA board for the Zynq purchase drops the total to about \$965. The Rogers PCB is the cost driver and the item most likely to cause schedule problems, since every revision is another board order and another ten-day wait, so we keep early iteration on cheap FR-4 and commit to the Rogers laminate only once the layout is stable.

\subsubsection*{Implementation Issues}

RF design is unforgiving, and the antenna and front end together are the critical path for the whole project, so we are budgeting for at least two board revisions on their account. We need bench access to a vector network analyzer and a spectrum analyzer for bring-up, which the department lab provides. On the regulatory side, we stay within the FCC Part 15 UWB emission mask: transmit power stays very low, and no license is required. Building a high-power radar in a licensed band is not an option we are considering. There are no moving parts and no mechanical hazards. The energy budget is not a concern, because the transmit power is small and the display and processor are the dominant loads.

FMCW radar in this frequency range is otherwise well-documented territory. MIT's 6.S062 ``coffee-can radar'' and Henrik Forsten's open 6 GHz FMCW radar project are both public end-to-end designs, and integrated PLL/VCO parts such as the ADF4159, paired with the HMC series, are available at reasonable cost. MIT's WiTrack project already demonstrated through-wall localization of a single moving person in roughly this band with about 1.8 GHz of bandwidth\footnote{\url{https://witrack.csail.mit.edu/}}\footnote{\url{https://witrack.csail.mit.edu/witrack-paper.pdf}}, so we are re-implementing a known result on a budget, not inventing a new one. The hard and novel work for us is the antenna array, the moving-target indication through a real wall, and getting the pipeline to run in real time; none of those is an open research question, they are all work, and this team can do a large amount of work.

\subsection*{Evaluation and Testing}

The minimum measure of success is to detect and range a single person walking behind one interior drywall wall, with range error under about 50 cm. The next target is to add angle of arrival, so that the device tracks that person's position within the forward sector and the blip moves on the screen in real time as they walk.

The test method is as follows. A person walks a marked path in the far room while a camera in that room, timestamp-synchronized to the radar, records ground truth. We compare the estimated track against the actual track and extract detection range, range accuracy, angular accuracy, update rate, and the number of people we can resolve at once.

We will be honest in the report about the real-world caveats. The device only detects moving people, because the static subtraction erases anyone holding perfectly still; a breathing-detection mode is a stretch goal that would recover some of them. Wall material changes the propagation speed, so range requires a per-wall calibration adjustment, and we will not pretend the device works without one. Multiple walls and exterior brick are out of scope. Full imaging, meaning an actual silhouette or skeleton, is explicitly not what this project is; that would take years of work and a trained neural network. We track blips.

\newpage

\section*{Proposal 2 (Unrealistic): Head-Mounted Aurora Viewer}

\subsection*{High-Level Description}

A visor that lets you see auroae the naked eye cannot. The human eye integrates light over roughly a tenth of a second, so a faint aurora appears as a dim grey smudge with no color. A camera can integrate for five seconds and pull out bright, structured green and purple curtains. This device performs that integration live and puts the result on a display in front of your eyes.

This idea originated when I first went aurora viewing here in Michigan, and was confused when everyone else began pulling out thier phones and cameras to take pictures while I could not see anything in the sky. I was able to capture the faint aurora with a long exposure shot on my phone, but I thought being able to view the faint and otherwise invisible aurora in real time would be very cool. 

The finished device is a visor or a pair of glasses. A low-light camera like the type used in dash cams (some fancy Sony sensor that works well with motion and low light) at the top, two micro-OLED displays, an IMU on the frame, and a compute pack on the headband or belt worn around the waist. It captures frames continuously, uses the IMU together with the camera stream to register each new frame against the running stack so that head motion cancels out, and integrates about five seconds of exposure. The stacked, denoised, color-boosted image is sent to the displays at a the highest framerate we can get, which will probably be in the 1 to low 2 digits. The compute pack (probably something along the lines of a Jetson Orin) may be quite heavy, hence why we suggest it could be waist mounted. 

\subsection*{Implementation}

\subsubsection*{Rough Timeline}

\begin{center}
\small
\begin{tabular}{|c|p{13.5cm}|}
\hline
9/10--10/1 & Part selection and laptop prototype. Select and purchase the camera, compute module, IMU, and displays. Build a prototype of the software stack running on a laptop and simulate it against night sky footage, and finalize this proposal. \\
\hline
10/1--10/22 & Carrier PCB. Design the board, covering power, IMU, camera interface, and display drivers, and order it. \\
\hline
10/22--11/19 & Port to the compute module. Get the pipeline running on the jetson at a few frames per second and optimize as needed. \\
\hline
11/12--12/3 & Visor integration and field testing. Integrate the visor, run field tests on clear nights, and tune denoising and exposure. \\
\hline
12/3--12/10 & Design the poster and prepare for the expo. Use an indoor simulated-sky demo as a fallback since aurora cannot be scheduled. Optionally take the teaching team out for demo if timing is convenient. \\
\hline
\end{tabular}
\end{center}

\subsubsection*{Rough Budget}

\begin{center}
\small
\begin{tabular}{p{6.5cm} p{5.5cm} c}
\hline
Category & Examples & Est. \$ \\
\hline
Compute module & Jetson Orin Nano & 250 \\
Low-light camera & Sony IMX908 & 250 \\
Displays & ECX334C Clone & 100\footnotemark \\
IMU, power, etc. & & 50 \\
Carrier PCB & & 100 \\
Optics, visor frame, battery, enclosure & & 200 \\
\hline
\textbf{Total} & & \textbf{950} \\
\hline
\end{tabular}
\end{center}
\footnotetext{Sony ECX334C MicroOLED display clones could be had for \$50 in China. Used in my 373 project.}

\subsubsection*{Implementation Issues}

The compute module runs hot and draws 2 digit wattage constantly, so we will need a big battery and some active cooling. 

Optics and camera selection is very finicky and iterative. There will be an extreme amount of sensor tuning and experimentation with filters to get an actually usable output. There's also the question of using the camera stream to do stabilization, which means some sort of sensor fusion algorithm, which is difficult and finicky to implement too. 

CV will be a very significant part of this project, so much so that you could argue that it becomes a ML project, not an embedded project.

\subsection*{Evaluation and Testing}

We would compare the stacked output's signal-to-noise ratio against a single frame, and against a tripod-mounted long-exposure camera shooting the same patch of sky. 

Success is pulling a Kp 3 aurora into clear, colored visibility when the naked eye can barely see anything. As a convenient indoor test, we can project a dim, slowly varying synthetic aurora onto a screen in a dark room and confirm the device recovers it while a bare camera frame does not. 

Eventually, real world testing should ideally be done. I captured my aurora at Pinckney Recreation Area about 30 minutes North by car of Ann Arbor. Go when there are clear skies and a high enough Kp value reported by NOAA. 

\end{document}
